...
class ScmDashboardScreen extends ConsumerWidget {
  ...
  Widget _operationalSnapshotCard() {
    return FutureBuilder<Map<String, int>>(
      future: ErpCrmIntelligenceService.instance.getOperationalDashboardSnapshot(
        availableStock: 147,
        lowStockItems: 18,
      ),
      builder: (context, snapshot) {
        ...
        return Container(
          ...
          child: Wrap(
            ...
            runSpacing: 10,
            children: [
              _metricChip('Orders', '${data['totalOrders'] ?? 0}'),
              _metricChip('Available Stock', '${data['availableStock'] ?? 0}'),
              _metricChip('Low Stock', '${data['lowStockItems'] ?? 0}'),
            ],
          ),
        );
      },
