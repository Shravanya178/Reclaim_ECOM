...
class _ProductCardState extends ConsumerState<_ProductCard>
  ...
  Future<void> _loadDemandSignals() async {
    final intelligence = ErpCrmIntelligenceService.instance;
    final highDemand = await intelligence.isHighDemand(widget.p.name);
    final secondsLeft = await intelligence.reservationSecondsLeft(widget.p.name);
    if (!mounted) return;
    setState(() {
      _isHighDemand = highDemand;
      _reservationSeconds = secondsLeft;
    });
  }
  ...
  void _addToCart() async {
    ...
    if (!reserved) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('This item is currently reserved for another buyer.'),
    ...
    final holdSeconds = await intelligence.reservationSecondsLeft(widget.p.name);
    await intelligence.trackInteraction(productId: widget.p.name, weight: 3);
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Row(children: [
          ...
          const SizedBox(width: 8),
          Expanded(
              child: Text('${widget.p.name} added to cart! Reserved for 2 minutes.',
      ...
      setState(() => _reservationSeconds = holdSeconds);
    }
    _loadDemandSignals();
                    ...
                    child: GestureDetector(
                      onTap: () async {
                        await ErpCrmIntelligenceService.instance
                            .trackInteraction(productId: widget.p.name, weight: 1);
                        if (!mounted) return;
                        showDialog(
                          context: context,
                      ...
                      children: [
                        ...
                        if (_reservationSeconds > 0)
                          Container(
                            ...
                            child: Text(
                              'Reserved for ${( _reservationSeconds / 60).ceil()} min',
                              style: const TextStyle(
                                fontSize: 11,
                                fontWeight: FontWeight.w700,
          ...
          Container(
            ...
            child: Row(children: [
              ...
              ElevatedButton.icon(
                onPressed: () async {
                  final intelligence = ErpCrmIntelligenceService.instance;
                  final reserved = await intelligence.reserveItem(p.name);
                  if (!reserved) {
                  ...
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                    content: Row(children: [
                      ...
                      const SizedBox(width: 8),
                      Expanded(child: Text('${p.name} added and reserved for 2 minutes!', style: const TextStyle(fontWeight: FontWeight.w600))),
                    ]),
                    backgroundColor: AppTheme.primaryGreen,
                    behavior: SnackBarBehavior.floating,
